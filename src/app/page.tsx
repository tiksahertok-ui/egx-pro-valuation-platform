...
function StockDetailPanel({ detail, onClose }: { detail: StockDetail; onClose: () => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {stock.price <= 0 && (
        <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center gap-2">
          <Info className="w-4 h-4 text-slate-500 shrink-0" />
          <p className="text-xs text-slate-600 dark:text-slate-300">
            This stock is listed on EGX but has no price or financial data in our database yet. Click <strong>Refresh</strong> to fetch the latest data.
          </p>
        </div>
      )}
      ...
      {/* Price & Key Metrics */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className={`${stock.price > 0 ? 'bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-900/10 border-emerald-200 dark:border-emerald-800' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700'}`}>
          <CardContent className="p-3">
            <p className="text-xs text-emerald-600 dark:text-emerald-400">Current Price</p>
            <p className="text-xl font-bold text-emerald-700 dark:text-emerald-300">{stock.price > 0 ? formatPrice(stock.price) : 'No data'}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
      ...
      {valuation && (
        <Card className="border-2 border-emerald-200 dark:border-emerald-800 bg-gradient-to-r from-emerald-50/50 to-white dark:from-emerald-900/10 dark:to-slate-900">
          ...
          <CardContent>
            {valuation.confidenceScore > 0.5 ? (
              <>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                  {/* Graham Number - Prominently Displayed */}
                  {(() => {
                    const grahamModel = valuation.models.find(m => m.model === 'graham');
                    if (grahamModel && grahamModel.confidence > 0.5 && grahamModel.fairValue > 0) {
                      return (
                        <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-3 border border-amber-200 dark:border-amber-800">
                          ...
                          <p className="text-2xl font-bold text-amber-700 dark:text-amber-300">{formatPrice(grahamModel.fairValue)}</p>
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            {grahamModel.upsideDownside > 0 ? '+' : ''}{safeToFixed(grahamModel.upsideDownside, 1)}% vs price
                          </p>
                        </div>
                      );
                    }
              ...
              <div className="mb-4 p-4 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-semibold text-amber-700 dark:text-amber-300">Fair value estimate is not available</p>
                </div>
                <p className="text-xs text-amber-600 dark:text-amber-400">
                  Confidence level is below 50% ({safeToFixed(valuation.confidenceScore * 100, 0)}%). More financial data is needed for reliable valuation.
                </p>
              </div>
            )}

            {/* Model Results Table - Always shown */}
                  ...
                  {valuation.models.map((m) => (
                    <TableRow key={m.model}>
                      <TableCell className="font-medium text-sm">
                        {m.modelName}
                        {m.model === 'graham' && m.confidence > 0.5 && m.fairValue > 0 && (
                          <span className="ml-1 text-[9px] text-amber-600 dark:text-amber-400 font-normal">★</span>
                        )}
                      </TableCell>
                      ...
                      <TableCell className="hidden md:table-cell">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full ${m.confidence > 0.5 ? 'bg-emerald-500' : 'bg-amber-400'}`}
      ...
      {!valuation && stock.price <= 0 && (
        <Card className="border-2 border-dashed border-slate-300 dark:border-slate-700">
          <CardContent className="p-6 text-center">
            <Calculator className="w-8 h-8 mx-auto mb-2 text-slate-400" />
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">No Valuation Available</p>
            <p className="text-xs text-slate-400 mt-1">
              This stock has no price or financial data yet. Refresh data to compute valuations.
            </p>
          </CardContent>
        </Card>
      )}
